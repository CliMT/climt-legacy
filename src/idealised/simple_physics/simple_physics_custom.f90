...
subroutine simple_physics_func (pcols, pver, dtime, lat, t, q, u, v, pmid, pint, &
        ...
        use_ts_ext, ts) bind(c,name='dcmipSimplePhysics')
...
!JOY adding new input variables
    ...
    integer, intent(in) :: do_qflux     ! Switch for surface latent heat fluxes
    integer, intent(in) :: do_momflux   ! Switch for surface momentum fluxes
    integer, intent(in) :: do_tflux     ! Switch for surface sensible heat fluxes
